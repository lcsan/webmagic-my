...
public class PhantomjsDownloader implements Downloader {
    ...
    private int sleepTime = 500;// 3s
    private PhantomjsAction action = null;
    private WebDriverPool webDriverPool = new WebDriverPool();

    public PhantomjsDownloader() {
    }

    public PhantomjsDownloader(int sleepTime, WebDriverPool pool) {
        this(sleepTime, pool, null);
    }

    public PhantomjsDownloader(int sleepTime, WebDriverPool pool, PhantomjsAction action) {
    ...
    public void setOperator(PhantomjsAction action) {
        this.action = action;
    }
    ...
    public Page download(Request request, Task task) {
        ...
        PhantomjsAction reqAction = (PhantomjsAction) request.getExtra("action");
        if (reqAction != null) {
        ...
        page.setHtml(new Html(UrlUtils.canonicalizeUrl(content, webDriver.getCurrentUrl())));
