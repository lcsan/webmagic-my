...
@ExtractBy(value = "json('$..thirdLevelCategory[*]')", multi = true)
public class Eexamples3 implements AfterExtractor {

	@ExtractBy("json('$..count')")
	private int count;

	@ExtractBy("json('$..filter').replace('\"label.*?\\}','\"namespace\":\"THIRD_LEVEL_CATEGORY\"\\}')")
	...
	public void afterProcess(Page page) {
		...
		if (count > 10000) {
			...
			Request req = new Request("https://matmatch.com/search?3");
